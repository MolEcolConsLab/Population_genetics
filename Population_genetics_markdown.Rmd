...
in_file = ""
popmap <- "" #Use whitelist of final indvs
```


## Read in data and create genind object with populations
```{r, echo = FALSE}
in_vcf <- read.vcfR(in_file, convertNA = T)
popmap <- read_tsv(popmap, col_names = c("Indiv", "STRATA"))

#Convert to genlight object
genlight <- vcfR2genlight(in_vcf) 
#Make sure population labels are in the correct order to match the individuals
popmap.filt <- genlight@ind.names %>% as_tibble_col(column_name = "Indiv") %>% 
  left_join(popmap, by = "Indiv") %>% 
  pull(STRATA)

genlight@pop <- factor(popmap.filt) # This line adds the population data.

#Convert to genind object
genind <- vcfR::vcfR2genind(in_vcf)
genind@pop <- factor(popmap.filt) # This line adds the population data. 
table(popmap.filt) # Print the total number of samples used in the analyses per population.
```
