...
whitelist.indvs <- read.delim("", header = F) %>% pull()
...
popmap.orig <- read_tsv("", col_names = F) %>% 
...
# Read in a "regions" file. This file should have at a minimum a column for Population, latitude, and longitude
region <- read.csv("", header = T)
...
Based on the output of the plot above, we select the optimal value of K and observe the number of samples per cluster. Set k to the value with the lowest BIC score.
```{r, echo=TRUE}
k <- 5
...
*Now use the first DPCA to inform a secondary DPCA*

**Q:** Why are we calling this a prior? Aren't we just empirically determining the best number of PCs to retain for DAPC?
...
ggscatter(dapc_prior, x = "LD1", y = "LD2", shape = 21, fill = "SITE", size = 3,
          xlab = "DA1 (30.9%)", ylab = "DA2 (29.9%)", palette = pop_cols)
